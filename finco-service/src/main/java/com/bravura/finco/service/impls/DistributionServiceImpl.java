...
public class DistributionServiceImpl implements DistributionService {
    ...
    public Object callDistributionProduct(FincoResponse fincoResponse) {
        ...
        if (systemCache.getIfPresent(DSTR_ACCESS_TOKEN) == null) {
            Optional<AuthToken> bearer = Optional.ofNullable(webClient
                    .get()
                    .uri(bearerURI)
                    .retrieve()
                    .bodyToMono(AuthToken.class)
                    .block());
            systemCache.put(DSTR_ACCESS_TOKEN, bearer.orElseThrow(
                        () -> new TechnicalException("Auth token not found.")
                    )
                    .getAccessToken());
        }
